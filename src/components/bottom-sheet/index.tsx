...
export type BottomSheetRefProps = {
  ...
  isVisible: (v: boolean) => void;
};

const BottomSheet = React.forwardRef<BottomSheetRefProps, BottomSheetProps>(({ children, height }, ref) => {
  const translateY = useSharedValue(0);
  const visible = useSharedValue(false);

  const scrollTo = useCallback((destination: number) => {
    "worklet";
    visible.value = destination !== 0;
  ...
  const gesture = Gesture.Pan()
    ...
    .onEnd(event => {
      if (translateY.value > -height && event.velocityY > 1) {
        scrollTo(0);
      }
    });
